...
auth.onAuthStateChanged((user) => {
  ...
  if (adminLink) {
    adminLink.style.display = (user && (user.email === 'jack1122@freelightmods.com')) ? 'inline-flex' : 'none';
  }
});
...
async function loadAppsData() {
  ...
  try {
    unsubscribe = db.collection('apks').orderBy('dateAdded', 'desc').onSnapshot((snapshot) => {
      appsData = [];
      ...
      if (appsData.length === 0) {
        grid.innerHTML = `<div class="loading-state" style="grid-column:1/-1"><i class="fas fa-cloud"></i><br>No APKs uploaded yet</div>`;
        return;
      }
...
if (aiToggle && aiChat) {
  aiToggle.addEventListener('click', () => { aiChat.style.display = 'block'; aiToggle.style.display = 'none'; });
  document.getElementById('aiClose')?.addEventListener('click', () => { aiChat.style.display = 'none'; aiToggle.style.display = 'flex'; });
...
function showTyping() {
  ...
  div.innerHTML = `<i class="fas fa-robot"></i><div class="typing"><span></span><span></span><span></span></div>`;
...
// ... (Existing code for Theme, Auth, Data Loading) ...
...
// ... (Rest of your existing public.js code) ...